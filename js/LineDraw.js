

const solutionPoints = [
  [234,2],[234,10],[202,10],[202,26],[282,26],[282,42],[298,42],[298,58],[282,58],[282,74],[266,74],[266,90],[250,90],[250,138],[234,138],[234,154],[218,154],[218,122],[234,122],[234,90],[218,90],[218,106],[202,106],[202,186],[186,186],[186,202],[170,202],[170,170],[186,170],[186,154],[170,154],[170,138],[122,138],[122,170],[106,170],[106,186],[122,186],[122,234],[106,234],[106,218],[90,218],[90,202],[74,202],[74,218],[58,218],[58,282],[106,282],[106,298],[122,298],[122,314],[106,314],[106,330],[122,330],[122,346],[106,346],[106,362],[90,362],[90,394],[74,394],[74,410],[106,410],[106,426],[138,426],[138,474],[154,474],[154,458],[170,458],[170,474],[186,474],[186,410],[202,410],[202,442],[234,442],[234,474],[250,474],[250,482]
];

const startBtn = document.getElementById('start-btn');
const mazeImg = document.querySelector('image');
const svg = document.querySelector('svg');


const solutionLine = document.createElementNS('http://www.w3.org/2000/svg', 'polyline');
solutionLine.setAttribute('id', 'solution-anim');
solutionLine.setAttribute('fill', 'none');
solutionLine.setAttribute('stroke', '#ff0000');
solutionLine.setAttribute('stroke-width', '2');
solutionLine.setAttribute('stroke-linecap', 'square');
solutionLine.setAttribute('stroke-linejoin', 'round');
solutionLine.setAttribute('points', solutionPoints.map(p => p.join(',')).join(' '));
solutionLine.setAttribute('class', 'draw-line');
svg.appendChild(solutionLine);


const length = solutionLine.getTotalLength();
solutionLine.style.strokeDasharray = length;
solutionLine.style.strokeDashoffset = length;
solutionLine.style.visibility = 'hidden';


startBtn.addEventListener('click', () => {
  
  solutionLine.style.animation = 'none';
  
  if (solutionLine.parentNode) {
    solutionLine.parentNode.removeChild(solutionLine);
    svg.appendChild(solutionLine);
  }
  solutionLine.style.strokeDashoffset = length;
  solutionLine.style.visibility = 'visible';
  void solutionLine.offsetWidth;
  solutionLine.style.animation = 'draw-line 10s linear forwards';
});

solutionLine.addEventListener('animationend', () => {
  solutionLine.style.visibility = 'hidden';
  solutionLine.style.animation = 'none';
  solutionLine.style.strokeDashoffset = length;
});